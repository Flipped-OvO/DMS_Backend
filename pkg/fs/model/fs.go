...
type (
	...
	FinancialIndex struct {
		...
		NpSR               int64 `gorm:"column:np_s_r"`                // 净利润率
		GpR                int64 `gorm:"column:gp_r"`                  // 毛利率(GM)
		...
		TlTaR              int64 `gorm:"column:tl_ta_r"`               // 资产负债率
		LwiTaR             int64 `gorm:"column:lwi_ta_r"`              // 有息负债率
		CabbTclR           int64 `gorm:"column:cabb_tcl_r"`            // 货币资金占流动负债比率
		CR                 int64 `gorm:"column:c_r"`                   // 流动比率
		GR                 int64 `gorm:"column:q_r"`                   // 速动比率
		FaTaR              int64 `gorm:"column:fa_ta_r"`               // 固定资产占总资产比率
		LvR                int64 `gorm:"column:lv_r"`                  // 清算价值比率
		Fcf                int64 `gorm:"column:fcf"`                   // 自由现金流量
		CrfscaplsOiR       int64 `gorm:"column:crfscapls_oi_r"`        // 销售商品提供劳务收到的现金对营业收入的比率
		NcffoaOpR          int64 `gorm:"column:ncffoa_op_r"`           // 经营活动产生的现金流量净额对营业利润的比率
		NcffoaNpR          int64 `gorm:"column:ncffoa_np_r"`           // 经营活动产生的现金流量净额对净利润的比率
		CrfscaplsTaR       int64 `gorm:"column:crfscapls_ta_r"`        // 销售商品提供劳务收到的现金对总资产的比率
		NcffoaFaR          int64 `gorm:"column:ncffoa_fa_r"`           // 经营活动产生的现金流量净额对固定资产的比率
	}
)
